  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gray-50 text-gray-900">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
              ...
              <button
                ...
                className={`px-3 py-1 rounded-full text-sm font-medium ${
                  autoRefresh && shouldPoll ? 'bg-red-600 text-white' : 
        ...
        {/* Banner de Status */}
        <div className={`border-b ${shouldPoll ? 'bg-red-600 border-red-500' : 'bg-orange-600 border-orange-500'}`}>
          <div className="container mx-auto px-4 py-2">
            <div className="flex items-center justify-center gap-2 text-sm">
              <span className={`px-2 py-1 rounded text-xs font-bold ${
                shouldPoll ? 'bg-red-800 text-red-100' : 'bg-orange-800 text-orange-100'
              }`}>
                {shouldPoll ? 'AO VIVO' : 'AGUARDANDO'}
              </span>
              <p className={`text-center ${shouldPoll ? 'text-red-100' : 'text-orange-100'}`}>
                {shouldPoll 
            ...
            <div className="container mx-auto px-4 py-4">
              <div className="text-center">
                ...
                <div className="grid md:grid-cols-3 gap-4 text-sm">
                  <div>
                    <p className="text-gray-500">Grande Prêmio</p>
                    <p className="font-semibold text-gray-900">{nextRace.name}</p>
                  </div>
                  <div>
          ...
          {activeTab === 'timing' ? (
            shouldPoll ? (
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2">
                  <div className="bg-white rounded-lg shadow-lg overflow-hidden">
                    <div className="p-4 border-b border-gray-200">
                      <h2 className="text-lg font-bold text-gray-900">🏎️ Classificação</h2>
                    </div>
                      ...
                      <div className="overflow-x-auto">
                        <table className="w-full">
                          <thead className="bg-gray-100">
                            <tr>
                              <th className="px-4 py-2 text-left text-sm font-medium text-gray-700">Pos</th>
                              <th className="px-4 py-2 text-left text-sm font-medium text-gray-700">Piloto</th>
                              <th className="px-4 py-2 text-left text-sm font-medium text-gray-700">Equipe</th>
                            </tr>
                          </thead>
                          <tbody>
                            {data.standings.map((driver) => (
                              <tr key={driver.driverNumber} className="border-b border-gray-200 hover:bg-gray-50">
                                <td className="px-4 py-3 text-sm text-gray-900">{driver.position}</td>
                                <td className="px-4 py-3">
                                  <div className="flex items-center gap-2">
                                    <span className="font-medium text-gray-900">{driver.driverName}</span>
                                    <span className="text-gray-500 text-sm">{driver.driverAcronym}</span>
                                  </div>
                                </td>
                                <td className="px-4 py-3">
                                  <div className="flex items-center gap-2">
                                    ...
                                    />
                                    <span className="text-gray-700">{driver.teamName}</span>
                                  </div>
                                </td>
                              </tr>
                            ))}
                ...
                {data?.hasGuesses && (
                  <div className="lg:col-span-1">
                    <div className="bg-white rounded-lg shadow-lg overflow-hidden">
                      <div className="p-4 border-b border-gray-200">
                        <h2 className="text-lg font-bold text-gray-900">🏆 Ranking ao Vivo</h2>
                      </div>
                    <div className="p-4">
                      {data?.liveRanking
                            ...
                            <div className="flex items-center justify-between mb-2">
                              ...
                              <div className="text-right">
                                <div className="text-sm text-gray-600">
                                  Pontos: <span className="font-bold text-gray-900">{ranking.currentScore.toFixed(3)}</span>
                                </div>
                                ...
                                return (
                                  <div 
                                    ...
                                    className={`text-center p-1 rounded ${
                                      currentPosition === pos + 1 ? 'bg-green-600 text-white' :
                                      Math.abs(positionDiff || 0) <= 3 ? 'bg-blue-600 text-white' :
                                  ...
                                  >
                                    ...
                                    {positionDiff !== null && (
                                      <div className={`text-xs ${
                                        positionDiff === 0 ? 'text-green-600' :
                                        positionDiff > 0 ? 'text-red-600' :
                                        'text-blue-600'
                                      }`}>
              ...
              <div className="text-center py-12">
                ...
                <h2 className="text-2xl font-bold text-gray-900 mb-4">Live Timing Pausado</h2>
              ...
              <div className="flex flex-wrap items-center justify-between gap-4 text-sm">
                <div className="flex items-center gap-4">
                  <div>
                    ...
                    <span className="font-medium text-gray-900">{data.session.session_name}</span>
                  </div>
                  <div>
                    <span className="text-gray-500">Local:</span>{' '}
                    <span className="font-medium text-gray-900">{data.session.location}, {data.session.country_name}</span>
                  </div>
                </div>
  ...
  if (!participants.length) {
    return (
      <div className="text-center py-8">
        <div className="text-gray-400 text-4xl mb-4">📊</div>
        <p className="text-gray-600">Aguardando dados de timing...</p>
      </div>
    );
  }
        ...
        {participants.map((p) => (
          <div
            ...
            className={`rounded-lg border p-3 bg-white ${
              p.isLeader ? 'border-yellow-500 ring-1 ring-yellow-500/20' : 'border-gray-200'
            }`}
          >
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${
                  p.isLeader ? 'bg-yellow-500 text-black' : 'bg-gray-600 text-white'
                }`}>
                  {p.position}
                </span>
                <div>
              ...
              <div className="text-right">
                <p className="text-sm font-mono text-emerald-600">{p.gap}</p>
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div className="text-center">
                <p className="font-mono text-blue-600">{p.lastLap}</p>
                <p className="text-gray-500">Última</p>
              </div>
              <div className="text-center">
                <p className="font-mono text-purple-600">{p.bestLap}</p>
          ...
          <tbody>
            {participants.map((p) => (
              <tr key={p.id} className="border-b border-gray-200 hover:bg-gray-50">
                ...
                <td className="p-3 text-gray-700">{p.team}</td>
                <td className="p-3 text-center font-mono text-emerald-600">{p.gap}</td>
                <td className="p-3 text-center font-mono text-blue-600">{p.lastLap}</td>
                <td className="p-3 text-center font-mono text-purple-600">{p.bestLap}</td>
              </tr>
            ))}</tbody>
        </table>
      </div>
...
function RaceControl({ messages }: { messages: LiveTimingData['raceControl'] }) {
  if (!messages.length) {
    return (
      <div className="text-center py-8">
        <div className="text-gray-400 text-4xl mb-4">🏁</div>
        <p className="text-gray-600">Nenhuma mensagem de controle</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      {messages.map((msg, index) => (
        <div key={`${msg.date}-${index}`} className="bg-white rounded-lg p-4 border-l-4 border-blue-500 border border-gray-200">
          ...
          <p className="mt-2 text-sm sm:text-base text-gray-900">{msg.message}</p>
        </div>
      ))}
    </div>
  );
