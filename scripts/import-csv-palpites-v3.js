...
function identifyRaceColumns(headerLine) {
  ...
  for (let i = 0; i < header.length; i++) {
    ...
    if (cell && GP_MAPPING[cell]) {
      // Estrutura correta: [País], [vazio], [posição], [piloto grid], [pts grid], [piloto race], [pts race]
      raceColumns[cell] = {
        name: cell,
        gpName: GP_MAPPING[cell],
        startCol: i,
        positionCol: i + 2,    // Coluna da posição (1º, 2º, etc.)
        gridPilotCol: i + 3,   // Coluna do piloto para Grid
        gridPtsCol: i + 4,     // Coluna dos pontos para Grid  
        racePilotCol: i + 5,   // Coluna do piloto para Race
        racePtsCol: i + 6      // Coluna dos pontos para Race
      };
    }
  }
...
function extractUserGuesses(csvContent) {
  ...
  console.log('🏎️ Corridas:', raceNames.join(', '));

  const users = [];
  const allGuesses = [];
  let currentUser = null;
  let currentUserGuesses = { qualifying: [], race: [] };
  ...
  for (let lineIndex = 2; lineIndex < lines.length; lineIndex++) {
    ...
    // Identificar se é uma nova linha de usuário (primeira coluna tem nome, segunda também)
    const userName = data[0];
    const isNewUser = userName && userName !== '' && data[1] === userName;
    
    if (isNewUser) {
      // Salvar palpites do usuário anterior se existir
      if (currentUser && (currentUserGuesses.qualifying.length > 0 || currentUserGuesses.race.length > 0)) {
        users.push({
          name: currentUser,
          qualifyingGuesses: currentUserGuesses.qualifying.length,
          raceGuesses: currentUserGuesses.race.length
        });
        
        allGuesses.push(...currentUserGuesses.qualifying, ...currentUserGuesses.race);
      }
      
      // Iniciar novo usuário
      currentUser = userName;
      currentUserGuesses = { qualifying: [], race: [] };
      console.log(`👤 Usuário: ${userName}`);
    }
    
    if (!currentUser) continue;

    // Para cada corrida, extrair palpites usando as colunas de posição da própria corrida
      ...
      if (raceInfo.positionCol < data.length) {
        ...
        if (positionMatch) {
          ...
          if (raceInfo.gridPilotCol < data.length) {
            ...
            if (driverId) {
              currentUserGuesses.qualifying.push({
              ...
              console.warn(`⚠️ Piloto Grid não mapeado: "${gridPilot}" (${currentUser} - ${raceInfo.name} - ${position}º)`);
            }
          }
          ...
          if (raceInfo.racePilotCol < data.length) {
            ...
            if (driverId) {
              ...
              console.warn(`⚠️ Piloto Race não mapeado: "${racePilot}" (${currentUser} - ${raceInfo.name} - ${position}º)`);
            }
          }
        }
      }
...
function generateSQLInserts(data) {
  ...
  let sql = '-- SQL para importar palpites do CSV (versão 3)\n';
...
async function main() {
  try {
    console.log('🚀 Iniciando processamento do CSV de palpites (Versão 3)...\n');
    ...
    const sqlPath = path.join(__dirname, '..', 'import-palpites-v3.sql');
    ...
    const jsonPath = path.join(__dirname, '..', 'palpites-processados-v3.json');
