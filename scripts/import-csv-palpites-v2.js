...
function identifyRaceColumns(headerLine) {
  ...
  for (let i = 0; i < header.length; i++) {
    ...
    if (cell && GP_MAPPING[cell]) {
      // Cada corrida tem 5 colunas: [corrida], [vazio], Grid, Piloto, Pts, Piloto
      raceColumns[cell] = {
        name: cell,
...
function extractUserGuesses(csvContent) {
  ...
  for (let lineIndex = 2; lineIndex < lines.length; lineIndex++) {
    ...
    // Identificar se é uma nova linha de usuário (primeira coluna tem nome)
    ...
    // Extrair posição da terceira coluna
    const positionText = data[2] || '';
...
function generateSQLInserts(data) {
  ...
  let sql = '-- SQL para importar palpites do CSV (versão 2)\n';
...
async function main() {
  try {
    console.log('🚀 Iniciando processamento do CSV de palpites (Versão 2)...\n');
    ...
    const sqlPath = path.join(__dirname, '..', 'import-palpites-v2.sql');
    ...
    const jsonPath = path.join(__dirname, '..', 'palpites-processados-v2.json');
